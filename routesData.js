...
// Get Single Item (First Way)

router.get('/api/employee/:id', (req, res) => {
    modelGlutenAdditive.findById(req.params.id, (err, data) => {
        if (!err) {
            res.send(data);
        } else {
            console.log(err);
...
router.put('/api/data/update/:id', (req, res) => {
    ...
    modelGlutenAdditive.findByIdAndUpdate(req.params.id, { $set: Item1 }, { new: true }, (err, data) => {
        if (!err) {
...
router.delete('/api/data/:id', (req, res) => {
    ...
    modelGlutenAdditive.findByIdAndRemove(req.params.id, (err, data) => {
        if (!err) {
            // res.send(data);
